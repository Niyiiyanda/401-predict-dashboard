...
def load_live_data():
    """Pulls all stats, including Opta xG/xA, natively from the official API."""
    try:
...
def get_roster(team_name, players_df, teams_df):
    ...
    # Filter active players and sort by form
    roster = players_df[
        (players_df['team'] == team_match.iloc[0]['id']) & 
        (players_df['status'] == 'a')
    ].sort_values(by='form', ascending=False).head(8)
    
    # Extract data directly from the unified native FPL dataset
    ui_data = pd.DataFrame({
        ...
        'Threat': roster['threat'],       # FPL's metric for shot volume/dangerous positions
        'ICT': roster['ict_index']        # Influence, Creativity, Threat index
    })
    
    return ui_data

# ==========================================
# 3. SIDEBAR & NAVIGATION (10 FIXTURES)
# ==========================================
